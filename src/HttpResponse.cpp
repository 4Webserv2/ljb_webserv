...
void HttpResponse::handleGet(const HttpRequest &req, const ServerBlock &serverBlock, const LocationBlock &location)
{
	if (location.getReturn().first != 0)
		return setResponseByStatus(location.getReturn().first, &serverBlock, location.getReturn().second);
...
std::string HttpResponse::findBestLocationMatch(const std::string &uri, const ServerBlock &serverBlock, LocationBlock &location) const
{
